...
import { AudioState } from "@/types/audio-state";
...
export default function SpotifySongProvider({ children }: SpotifySongProviderProps) {
  const [songs, setSongs] = useState<StatefulSong[]>([]);
  const [currentSongId, setCurrentSongId] = useState<string>();
  const [timePlayed, setTimePlayed] = useState<SongTimePlayed>({});
  const currentSong = useMemo(() => songs.find((s) => s.id === currentSongId), [songs, currentSongId]);

  const registerSong = useCallback(
    (newSong: Song) =>
      setSongs((previousSongs) => {
        if (!newSong.previewUrl || previousSongs.some((song) => song.id === newSong.id)) return previousSongs;
  ...
  const updatePlayedAmount = useCallback(() => {
    ...
    setTimePlayed((previousTimePlayed) => ({
      ...previousTimePlayed,
      [currentSong.id]: Math.round((audio.currentTime / (audio.duration - 1)) * 100),
    }));
  }, [currentSong]);
  ...
  const setAudioPaused = useCallback(
    (paused: boolean, ended: boolean = false) => {
      ...
      setSongs((previousSongs) => {
        ...
        const songIndex = newSongs.findIndex((song) => song.id === currentSong.id);
  ...
  const onAudioPlay = useCallback(() => setAudioPaused(false), [setAudioPaused]);

  const onAudioPause = useCallback(() => setAudioPaused(true), [setAudioPaused]);
  ...
  }, [currentSong, onAudioPlay, onAudioPause, onAudioEnded, updatePlayedAmount]);
