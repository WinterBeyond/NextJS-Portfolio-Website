...
const Repositories = ({ repositories }) => {
	...
	return (
		<section className="max-w-6xl mx-auto px-4 py-5 mt-20 rounded-lg bg-white dark:bg-transparent">
			<div className="flex flex-col md:flex-row justify-between items-center md:pt-20 mx-10">
				...
				<GithubIcon className="flex flex-row mb-10 px-8 py-4 rounded-md bg-gray-200 dark:bg-gray-900 shadow-lg text-xl space-x-2 items-center">
					<p>View GitHub</p>
				</GithubIcon>
			</div>
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto px-10 lg:-mt-10 gap-y-20">
				{repositories?.map((repository) => (
					<GithubCard repository={repository} key={repository.id} />
				))}
			</div>
		</section>
	);
